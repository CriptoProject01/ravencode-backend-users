...
async def login(
    ...
    Includes reCAPTCHA verification for bot protection.
    """
    # Verify reCAPTCHA token
    client_ip = request.client.host if request.client else None
    if not RecaptchaService.is_valid(login_data.recaptcha_token, client_ip):
        raise HTTPException(
...
async def register(
    ...
    # Verify reCAPTCHA token
    client_ip = request.client.host if request.client else None
    if not RecaptchaService.is_valid(register_data.recaptcha_token, client_ip):
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="reCAPTCHA verification failed. Please try again."
        )
